...
export default function EnterResult() {
  ...
  async function handleSubmit() {
    ...
    if (!set2.team1 || !set2.team2) { setError('Set 2 skorlarını giriniz.'); return; }
    if (needsTiebreak() && (!set3.team1 || !set3.team2)) { setError('Tiebreak skorlarını giriniz.'); return; }
    if (!winnerId) { setError('Kazananı seçiniz.'); return; }

    let score = set1.team1 + '-' + set1.team2 + ', ' + set2.team1 + '-' + set2.team2;
    if (needsTiebreak()) score += ', ' + set3.team1 + '-' + set3.team2 + ' (TB)';

    setSubmitting(true);
    const loserId = winnerId === selectedMatch.fromTeamId ? selectedMatch.toTeamId : selectedMatch.fromTeamId;
    ...
    try {
      ...
      await updateDoc(doc(db, 'matchRequests', selectedMatch.id), { status: 'completed', completedAt: serverTimestamp() });
      ...
      await addDoc(collection(db, 'matchHistory'), {
        ...
        winnerId, loserId, score, playedAt: serverTimestamp(),
      });
      await addDoc(collection(db, 'auditLog'), {
        action: 'result_entered', actor: 'player', targetId: selectedMatch.id,
        payload: { fromTeamName: selectedMatch.fromTeamName, toTeamName: selectedMatch.toTeamName, score, winnerId, loserId },
        ...
        <div style={{ paddingTop: 32, paddingBottom: 48 }}>
          ...
          {view === 'success' && (
            <div style={{ background: '#0f2a0f', border: '1px solid #4caf5044', borderRadius: 12, padding: '40px 24px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 13, color: '#4a7a4a', marginBottom: 24 }}>Sıralama güncellendi.</div>
          ...
          {view === 'enter-result' && selectedMatch && (
            <div>
              <div style={{ marginBottom: 28, padding: '16px 20px', background: '#162a16', borderRadius: 12, border: '1px solid #2e4a2e' }}>
                <div style={{ fontSize: 11, color: '#4caf50', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.06em' }}>Match</div>
              ...
              <div style={{ marginBottom: 28 }}>
                <p style={{ fontSize: 11, fontWeight: 600, marginBottom: 6, color: '#8bc34a', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Set scores</p>
                <p style={{ fontSize: 12, color: '#4a7a4a', marginBottom: 16, marginTop: 0 }}>
                  En fazla 2 set oynanır. 1-1 berabere kalınması durumunda, tiebreak seti oynanır (10'a ilk ulaşan kazanır).
                </p>

                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12, gap: 12 }}>
                  <div style={{ width: 100, fontSize: 11, color: '#4a7a4a', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    Set 1
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <div style={{ fontSize: 11, color: '#4a7a4a', marginBottom: 4, textAlign: 'center', width: 72 }}>{selectedMatch.fromTeamName.split(' ')[0]}</div>
                    <div style={{ width: 18 }}></div>
                    <div style={{ fontSize: 11, color: '#4a7a4a', marginBottom: 4, textAlign: 'center', width: 72 }}>{selectedMatch.toTeamName.split(' ')[0]}</div>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12, gap: 12 }}>
                  ...
                  {set1Winner() && (
                    <div style={{ fontSize: 11, color: '#8bc34a', fontWeight: 600 }}>
                      {set1Winner() === 'team1' ? selectedMatch.fromTeamName : selectedMatch.toTeamName} wins set
                    </div>
                  )}
                </div>

                <div style={{ display: 'flex', alignItems: 'center', marginBottom: 12, gap: 12 }}>
                  <div style={{ width: 100, fontSize: 11, color: set1Winner() ? '#4a7a4a' : '#2e4a2e', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                    Set 2
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  {set2Winner() && (
                    <div style={{ fontSize: 11, color: '#8bc34a', fontWeight: 600 }}>
                      {set2Winner() === 'team1' ? selectedMatch.fromTeamName : selectedMatch.toTeamName} wins set
                    </div>
                  )}
                </div>
                ...
                {needsTiebreak() && (
                  <div>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 100, fontSize: 11, color: '#c8e64a', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
                        Tiebreak
                      </div>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <div style={{ marginBottom: 28 }}>
                <p style={{ fontSize: 11, fontWeight: 600, marginBottom: 14, color: '#8bc34a', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Who won?</p>
              ...
              <button onClick={handleSubmit} disabled={submitting}
                style={{ width: '100%', padding: 16, borderRadius: 10, border: 'none', background: '#8bc34a', color: '#0f1f0f', fontSize: 14, fontWeight: 700, cursor: 'pointer', opacity: submitting ? 0.7 : 1, textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 8 }}>
